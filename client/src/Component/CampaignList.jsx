...
function CampaignList({ onEdit, broadcast }) {
    ...
    const totalPages = Math.ceil(campaign.length / limit);
    const currentData = campaign.slice((currentPage - 1) * limit, currentPage * limit);
    ...
    const handleBroadCast = (_id) => {
        broadcast(campaign[_id]);
    }
            ...
            <div className='mt-[20px] rounded-md p-[0px]  bg-white text-gray-600 flex flex-col justify-between '>
                    ...
                    <table className='table-auto w-full  '>
                        <thead>
                            <tr className='sticky top-0 z-10 bg-white border-b border-gray-300 text-nowrap text-center '>
                                ...
                                <th className=' pb-4'>Total Dilvered</th>
                            ...
                            {currentData.length === 0 ? (
                                ...
                                currentData.map((item) => (
                                    ...
                                    <tr key={item._id} className="text-center  font-semibold">
                                        <td className="text-left py-4  text-blue-500 hover:underline cursor-pointer" onClick={() => handleEdit(_id)}>{item.campaignName || '-'}</td>
                                        ...
                                        <td className="text-right ">
                                            ...
                                            <div className='flex items-center gap-2 justify-end'>
                                                ...
                                                <button type='button' className='px-2 py-1 border rounded-lg text-green-600 text-sm font-medium bg-green-50 hover:bg-green-100 cursor-pointer border-green-600 text-nowrap' onClick={() => handleBroadCast(item._id)}>Send Boradcast</button>
                                                <i className="fa-solid fa-pen-to-square bg-gray-100 p-2 rounded-lg text-blue-500 hover:text-blue-600 hover:bg-blue-100 cursor-pointer" onClick={() => handleEdit(item._id)} ></i>

                                                <i className="fa-solid fa-trash bg-gray-100 p-2 rounded-lg text-red-500 hover:text-red-600 hover:bg-red-100 cursor-pointer" onClick={() => handleCampaignDelete(item._id)}></i>
                                            </div>
                                        </td>
                                    </tr>
                                ))
                ...
                <div className='flex items-center justify-between mt-4 px-2 py-2  text-gray-700'>
                    <div className='flex items-center  font-semibold text-gray-600 gap-2'>
                        ...
                        <select value={limit} onChange={(e) => setLimit(Number(e.target.value))} className="border focus:outline-none border-gray-300 rounded px-1 py-1">
                            {[2, 3, 10, 20, 50].map((n) => (
